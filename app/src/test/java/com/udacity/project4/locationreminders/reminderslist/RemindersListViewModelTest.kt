...
class RemindersListViewModelTest {
    ...
    @Test
    fun setCurrentLocationPermissionState_retrievedAsPrecise() {
        // WHEN the user grants precise location permission
        remindersListViewModel.setCurrentLocationPermission(
            CurrentLocationPermission.PRECISE,
        )

        // THEN the system set the currentLocationPermission as PRECISE to proceed with the flow
        assertThat(
            remindersListViewModel.currentLocationPermission.getOrAwaitValue(),
            `is`(CurrentLocationPermission.PRECISE),
        )
    }

    @Test
    fun setCurrentLocationPermissionState_retrievedAsCoarse() {
        // WHEN the user grants coarse location permission
        remindersListViewModel.setCurrentLocationPermission(
            CurrentLocationPermission.COARSE,
        )

        // THEN the system set the currentLocationPermission as COARSE to not proceed with the flow
        assertThat(
            remindersListViewModel.currentLocationPermission.getOrAwaitValue(),
            `is`(CurrentLocationPermission.COARSE),
        )
    }

    @Test
    fun setCurrentLocationPermissionState_retrievedAsNotGranted() {
        // WHEN the user does not grant location permission
        remindersListViewModel.setCurrentLocationPermission(
            CurrentLocationPermission.NOT_GRANTED,
        )

        // THEN the system set the currentLocationPermission as NOT_GRANTED to not proceed with the flow
        assertThat(
            remindersListViewModel.currentLocationPermission.getOrAwaitValue(),
            `is`(CurrentLocationPermission.NOT_GRANTED),
        )
    }

    @Test
    fun setUserAvailableToSaveReminders_proceedToSaveAReminder() {
        // WHEN the user collects what is necessary to can save a reminder
        remindersListViewModel.setUserAvailableToSaveReminders()

        // THEN the system set the isAvailableToSaveAReminder as true to can click the add reminder button
        assertThat(remindersListViewModel.isAvailableToSaveAReminder.getOrAwaitValue(), `is`(true))
    }
}
